"""Generates the MOFQuantumNet pipeline architecture diagram as a standalone SVG file.

Run with: python pipeline_diagram.py
Writes to: outputs/pipeline_architecture.svg (README.md embeds it directly — GitHub and
every modern browser render SVG natively, so no PNG rasterization step is needed).

Previously a Streamlit page with a client-side "download as image" button; rewritten as a
plain script once Streamlit was removed from this project (see README.md's "Key
implementation decisions" — the web console under webapp/ replaced it). The diagram
content itself is unchanged, just the delivery mechanism.
"""

import os

_SVG = """<svg id="pipeline-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1260" width="1200" height="1260" font-family="Inter,ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,sans-serif">
  <defs>
...
def generate(output_path: str = "outputs/pipeline_architecture.svg") -> str:
...
if __name__ == "__main__":
    path = generate()
    print(f"Wrote pipeline diagram to {path}")
